...
RSpec.describe RuboCop::Cop::FactoryBotGuide::DynamicAttributeEvaluation, :config do
      ...
      it "registers an offense" do
        expect_offense(<<~RUBY)
          factory :event do
            ...
            ^^^^^^^^^^^^^^^^^^^^^ FactoryBotGuide/DynamicAttributeEvaluation: Use block syntax for attribute `event_date` because `Date.today` is evaluated once at factory definition time. Wrap in block: `event_date { Date.today }`
          end
        RUBY
      end
    end
      ...
      it "registers an offense" do
        expect_offense(<<~RUBY)
          factory :post do
            ...
            ^^^^^^^^^^^^^^^^^^^^^^^^^ FactoryBotGuide/DynamicAttributeEvaluation: Use block syntax for attribute `published_at` because `DateTime.now` is evaluated once at factory definition time. Wrap in block: `published_at { DateTime.now }`
          end
        RUBY
      end
    end
      ...
      it "registers an offense" do
        expect_offense(<<~RUBY)
          factory :user do
            ...
            ^^^^^^^^^^^^^^^^^^^^^^ FactoryBotGuide/DynamicAttributeEvaluation: Use block syntax for attribute `uuid` because `SecureRandom.uuid` is evaluated once at factory definition time. Wrap in block: `uuid { SecureRandom.uuid }`
          end
        RUBY
      end
    end
